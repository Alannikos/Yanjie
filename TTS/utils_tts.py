import sys
sys.path.append("/root/Yanjie/Components/TTS")
...
# 这里可以做成一个init
chat = ChatTTS.Chat()
chat.load(compile=False, source='custom', custom_path='/root/Yanjie/Components/TTS/weights/ChatTTS')  # 用这个最新的配置
...
def prepare_tts_generation_config():
    ...
    if model_type == "温柔御姐":
        return "/root/Yanjie/Components/TTS/weights/speaker/seed_742_restored_emb.pt"
    else:
        return "/root/Yanjie/Components/TTS/weights/speaker/seed_1089_restored_emb.pt"
...
def text2audio(text, speaker_type=None, oral=3, laugh=3, bk=3):
    ...
    if speaker_type == None:
        speaker = chat._encode_spk_emb(torch.load('/root/Yanjie/Components/TTS/weights/speaker/seed_1089_restored_emb.pt'))
    else:
    ...
    wav_save_path = str(Path("/root/Yanjie/Components/Work_dirs/TTS").joinpath(save_file).absolute())
