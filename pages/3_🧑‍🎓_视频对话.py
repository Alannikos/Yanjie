...
# from THG.utils_thg import show_video, load_thg_model, save_videos, generate_thg
...
def main():
    ...
    with st.sidebar:
        ...
        with tabs[1]:
            ...
            st.session_state.enable_show_text = st.toggle("是否显示文字", key="config_show_text")
            st.session_state.enable_thg = st.toggle("是否生成数字人", key="config_show_huamn")
    ...
    if prompt := st.chat_input('You are welcome to chat with me, please enter your question:'):
        ...
        print("重新运行audio_bytes函数")
        # 先对录音进行保存，然后转文字
        save_voice_path = save_wavs(audio_bytes)
        key_gen = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        audio_bytes = audio_recorder(
            text="",
            recording_color="#e8b62c",
            neutral_color="#6aa36f",
    ...
    elif (st.session_state.enable_thg):
        """
        生成数字人
        """
        pass
    ...
    st.video("/root/Project_Yanjie/Developing/THG/EchoMimic/output/20240814/1244--seed_420-512x512/a_echomimic_en_512x512_2_1244_withaudio.mp4", format='video/mp4')
